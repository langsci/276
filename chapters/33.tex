...
\author{Howard Lasnik\and Zach Stone\affiliation{University of Maryland College
Park}}
...
\cite{Chomsky2000}. \textcite[22]{Chomsky1993} formulated \gls{EC}\is{Extension
...
\ea\label{ex:key:33.2}\begin{tikzpicture}[baseline=(root.base)]
...
\ea\label{ex:key:33.3}\begin{tikzpicture}[baseline=(root.base)]
...
\ea\label{ex:key:33.5}\{\text{XP, $\beta$ XP, $\beta$ X YP, $\beta$ X Y ZP, $\beta$ X Y Z}\}
\z
...
LSLT\nocite{Chomsky1955}-style \gls{PM}\is{phrase marker}, the resulting set is never a superset
...
\ea\label{ex:key:33.6}\begin{tikzpicture}[baseline=(root.base)]
        \Tree [.\node(root){XP}; [.$\beta$ ] [.XP [.X ] [.YP [.Y ] [.ZP [.WP [.QP [.Q ] ] [.W ] ] [.Z$'$ [.Z ] ] ] ] ] ]
    \end{tikzpicture}
\z

The initial \gls{RPM}\is{reduced phrase marker} is \eqref{ex:key:33.10}:

\ea\label{ex:key:33.10}$\{\text{XP, X YP, X Y ZP, X Y WP Z, X Y QP W Z, X Y Q W Z$'$, X Y Q Z}\}$
\z

And the derived \gls{RPM}\is{reduced phrase marker} is \eqref{ex:key:33.11}:
...
LSLT\nocite{Chomsky1955} \gls{PM}\is{phrase marker} in this case):

\ea\label{ex:key:33.12}\begin{tikzpicture}[baseline=(root.base)]
    \Tree [.\node(root){XP}; [.X ] [.YP [.Y ] [.ZP [.Z ] ] ] ]
\end{tikzpicture}
\z
...
\ea\label{ex:key:33.14}\begin{tikzpicture}[baseline=(root.base)]
...
\ea\label{ex:key:33.20}\begin{tikzpicture}[baseline=(root.base)]
...
\ea\label{ex:key:33.21}\begin{tikzpicture}[baseline=(root.base)]
...
But now look again at the \gls{PM}\is{phrase marker} \eqref{ex:key:33.18} to
...
\glsunset{BPS}\glsdesc{BPS} (\gls{BPS}, \citealt{ColSta2016}
...
in \REF{ex:key:aczel}. We could similarly use the containment relation in
...
\cite{chomsky1999derivation} has a \enquote{valuation} version of \isi{agreement}, which is
...
\cite{frampton2000agreement} give an explicit architecture for \isi{agreement} as
...
Using LSLT\nocite{Chomsky1955} phrase markers\is{phrase marker}, constituents
