...
%    \begin{subtable}{.5\linewidth}
      \centering
%        \caption{IFFs in the verbal domain}
        \begin{tabular}{llll}
        ...
        \hspace{2em}
      \centering
%        \caption{IFFs in the nominal domain}
        \begin{tabular}{llll}
        ...
        \end{tabular}
%    \end{subtable}
        \caption{\glsreset{IFF}\Glspl{IFF}}
        \label{form-fs}
\end{table}
...
Leo-\Nom/Ø Tolstoy-\Acc{} \Aux.\Fsg{} read.\textsc{ppl.sg.m} \\
\trans `I'm reading Leo Tolstoy.'}
...
Leo-\Nom/Ø \Aux.\Fsg{} Tolstoy-\Acc{} read.\textsc{ppl.sg.m} \\
\trans `I was reading Leo Tolstoy.'}
\end{xlisti}
...
approach in full generality. It is thus, \emph{ceteris paribus}, more
...
For \cite{Roberts2010}, it is critical that pronominal \isi{clitics} in
Ser-Bo-Croatian\il{Serbo-Croatian} be D\tsp{min/max}. For \cite{Boskovic:2005},
...
of C\tsp{min} from the T-complex, viz.\ the
%\tuple{C$^{min}_{3}$,\lambda$_3$,t$_3$} chain, yields the
following structure:

\ea\label{4}
    \begin{tikzpicture}[baseline=(root.base), scale=.85, align=center]
...
\emph{int. al.}), J\tsp{min} has some \gls{IFF} and an
        ...
        \Tree   [.\node(root){Force\tsp{max}};
                    ...
                    [.Fin\tsp{max}
                        ...
                        [.T\tsp{max}
                            ...
                            [.T\tsp{max}
                                {T\tsp{min}\\
                                    $[^{\checkmark}$\emph{u}φ/D, \emph{i}T,$^{\checkmark}$\gls{EPP}$]$\\
                                    \textbf{smo}}
    ...
    \begin{tikzpicture}[scale=.85, baseline=(root.base)]
        \Tree
            [.\node(root){Force\tsp{max}};
                {Force\tsp{min}\\{}[\gls{EF}]}
                [.Fin\tsp{max}
                    [.{Fin\tsp{min}\\{}[\emph{u}D, $^{\checkmark}$\emph{u}T,\emph{i}C]}
                        {T\tsp{min}\\
                            \textbf{smo}}
                        {Fin\tsp{min}\\}
                    ]
    ...
    \Tree
        [.\node(root){Force\tsp{max}};
            ...
            [.Force\tsp{max}
                ...
                [.Fin\tsp{max}
                    [.{Fin\tsp{min}\\
                        $[^{\checkmark}$\emph{u}D, $^{\checkmark}$\emph{u}T, \emph{i}C]}
                        {T\tsp{min}\\
                            \textbf{smo}}
                        {Fin\tsp{min}\\}
                    ]
                    [.T\tsp{max}
                        $\tuple{\text{J^{\text{max}}_{[^{\checkmark}u\text{φ}]}}}$
                        [.T\tsp{max}
                            {$\tuple{\text{T\tsp{min}}}$\\
                            $[^{\checkmark}$\emph{u}φ/D, \emph{i}T,$^{\checkmark}$\gls{EPP}$]$\\
                            smo}
...
maintain, \emph{ceteris paribus}, is the raising of the defective T\tsp{min} as
...
\posscitet{RackRich2005} definition of the Closest available goal:
...
Axiom}, prohibits rightward \isi{movement}, \emph{qua} lowering, and is both
...
\begin{exe}
	...
	[.λ {A\tsp{min}\\{}[\emph{i}N, \emph{i}\Def{},$^{\checkmark}$\emph{u}φ, u\textsc{case}]}
	{P\tsp{min}\\{}[\emph{i}N, \emph{i}\Def{},$^{\checkmark}$\emph{u}φ, i\textsc{case:\Loc{}}]} ]]
	\end{tikzpicture}
	\hfill\null
\label{der1}
\end{exe}
...
\begin{exe}
    ...
    \begin{tikzpicture}[baseline=(root.base)]
        \Tree [.\node(root){N};
	[.P {A\tsp{min}\\{}[\emph{i}N, \emph{i}\Def{},$^{\checkmark}$\emph{u}φ,$^{\checkmark}$u\textsc{case}]\\\textbf{veliku}}
	{P\tsp{min}\\{}[\emph{i}N,$^{\checkmark}$\emph{u}φ, i\textsc{case:\Loc{}}]\\\textbf{u}} ]
	 [.N\tsp{max} λ\tss{1} [.N {N\tsp{min}\\{}[\emph{i}φ,$^{\checkmark}$\emph{u}\textsc{case}]\\\textbf{sobu}}
	$t_1$ ]]]
	\end{tikzpicture}
...
constitutes a \isi{defective goal} which obtains incorporation.\footnote{The
    fact that XLBE material is in \isi{focus} testifies to the definiteness of
    the AP. Unlike ordinary \gls{LBE}\is{left branch extraction}, XLBE
obligatorily displays a definiteness effect.}

\begin{exe}
	\ex
        \begin{tikzpicture}[scale=.9, baseline=(root.base)]
    \Tree [.\node(root){N\tsp{max}};
	[.P [.A\tsp{min} {P$^{min}_2$\\{}[\emph{i}N,$^{\checkmark}$\emph{u}φ, i\textsc{case:\Loc{}}]\\\textbf{u}}
	{A\tsp{min}\\{}[\emph{i}N, \emph{i}\Def{},$^{\checkmark}$\emph{u}φ,$^{\checkmark}$u\textsc{case}]\\\textbf{veliku}}  ]
	$t_2$ ]
	 [.N\tsp{max} λ\tss{1} [.N {N\tsp{min}\\{}[\emph{i}φ,$^{\checkmark}$\emph{u}\textsc{case}]\\\textbf{sobu}}
	$t_1$ ]]]
	\end{tikzpicture}
...
Upon final \isi{movement}, the adjective is a maximal category via a mechanism of
reprojection or Self \isi{Merge} (we remain agnostic or rather apathetic with regards
to this issue).

\begin{exe}
	\ex
        \begin{tikzpicture}[scale=.8, baseline=(root.base)]
        \Tree
            [.\node(root){N\tsp{max}};
                [.A\tsp{max}
                    [.A$^{\text{min}}_3$
                        {P$^{\text{min}}_2$\\
                        {}[\emph{i}N,$^{\checkmark}$\emph{u}φ, i\textsc{case:\Loc{}}]\\
                        \textbf{u}}
                    	{A\tsp{min}\\
                        {}[\emph{i}N, \emph{i}\Def{},$^{\checkmark}$\emph{u}φ,$^{\checkmark}$u\textsc{case}]\\
                        \textbf{veliku}}
                    ]
                ]
                [.N\tsp{max}
                    ...
                    [.N\tsp{max}
                        ...
                        [.N\tsp{max}
                            ...
                            [.N
                                {N\tsp{min}\\
                                {}[\emph{i}φ,$^{\checkmark}$\emph{u}\textsc{case}]\\
                                \textbf{sobu}}
                                $t_1$
                            ]
                        ]
...
arrive at a critically similar configuration. Since A\tsp{max}
adjoining the N-complex would not project, due to the nature of the NP-over-AP
status of Ser-Bo-Croatian\il{Serbo-Croatian},  P\tsp{min},
contained in \textsc{Compl}(N\tsp{min}), would excorporate to the
root, \emph{ceteris paribus}. A\tsp{min} would have its [\emph{u}φ]
features checked via c-selection of N and its $[u\textsc{case}]$ feature valued
presumably via the chain \tuple{N\tsp{min}[u\textsc{case: }],
N\tsp{min}[i\textsc{case:loc}]}. In case A\tsp{min} is
specified with a \textsc{[def]} feature, the features constitute a superset of
those on P\tsp{min} which would, in absence of \textsc{[def]} on
A\tsp{min}, otherwise excorporate to the root. This way, P is a
...
\tuple{A$^\text{{max}}_{1}$, \emph{t}\tss{1}}, and subsequent incorporation of
...
\tuple{P$^{\text{min}}_{2}$, \emph{t}\tss{2}} can be seen as breaching the
    ...
    \ex F\tss{\emph{v}\tsp{0}} $\subset$ F\tss{\emph{n}\tsp{0}}\\
        \begin{tikzpicture}[baseline]
            \Tree
             ...
             +(south:1).. node [fill=white] {$\checkmark$} (2.south);
...
types of Defective goals insofar as pronominal cliticisation\is{clitics} is concerned.
...
\begin{exe}
    ...
    \ex \isi{Agree} between Phase-Phase objects yielding incorporation via chain
        \tuple{α\tss{[+π]}, α\tss{[+π]}}
	\end{xlisti}
    ...
    chain \tuple{α\tss{[+π]}, α\tss{[-π]}}
    \end{xlisti}	\end{xlista}
\end{exe}

My account leaves the analysis of \ili{Romance} pronominal
cliticisation\is{clitics}, which \textcite{Roberts2010} analyses as involving a
...
and, to some extend, \ili{Slovenian}) have developed an overtly full-fledged
