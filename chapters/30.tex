...
of \figref{ex:multi2}, including the one Nunes' theory countenanced
in~(\ref{ex:covert}).
...
\begin{figure}[hb]
\caption{\color{red}Please provide a caption\label{ex:multi3}}
    \begin{tikzpicture}[baseline]
        \tikzset{level 1/.style={sibling distance=-80pt}}
        \Tree   [.\node(cp){CP};
                    [.CP$\dag$
...
As we've seen, \emph{which} and \emph{flower} have two paths in \figref{ex:multi3-2}, and so the largest $\Pi$ contains them both:
\begin{exe}
