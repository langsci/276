...
distinction between binary (cf.\ (\ref{ex:key:08.1}a) below) and ternary (cf.\
(\ref{ex:key:08.1}b) below) person-based systems (cf.\
...
\ea\label{bkm:Ref370478291}\label{ex:key:08.1}
\ea\label{ex:key:08.1a} \ili{Romanian} (personal knowledge)\\
\gll acest /  acel  copil\\
this  {} that  child\\
\glt \enquote*{This / That child}
\ex\label{ex:key:08.1b}Asturian \citep{ALIA:2001a}\\
...
further developed by the \enquote{Rethinking Comparative Syntax} (\emph{ReCoS})
research group led by Ian Roberts,\footnote{For information about the
\emph{ReCoS} project, including recent publications, see
http://recos-dtal.mml.cam.ac.uk/.} I shall explore (\cref{bkm:Ref370483064})
...
demonstrative system (cf.\ \tabref{tab:key:08.1}), in which referents which fall
...
(cf.  \figref{fig:key:1}) proposed by \citet{Benveniste:1946a}, argues that
...
\begin{figure}
    \caption{Effects of loss of \textsc{hic}}\label{fig:key:1}
    \begin{tikzpicture}
...
\eqref{ex:key:08.3} below; \citealt[416]{Nyrop:1925a}), Old \ili{Occitan}
...
Northern \ili{Italian} dialects also present another binary demonstrative\is{demonstratives} sys\-tem,
...
\ea\label{ex:key:08.7}Modern \ili{French} \citep{Smith:1995a}\\
\ea
\gll  Je  suis  là\\
I  am  there\\
...
In \figref{fig:key:1} we saw how, following \citet{Vincent:1999ab}, with the
...
(\ref{ex:key:08.8}a) below), though second-person \isi{deixis} could be marked separately where required
...
((\ref{ex:key:08.10}a) below; \citealt[104f]{ALIA:2001a}). Unlike in \ili{Romanian} where postnominal
...
modifiers (cf.\ (\ref{ex:key:08.10}c) below). A further difference is that whereas in \ili{Romanian} the
...
hence its incompatibility with contrastive \isi{focus} (cf.\ (\ref{ex:key:08.10}d) below;
\citealt{Roca:2009a}).
...
((\ref{ex:key:08.11}b) below) each of the three \isi{demonstratives} refers
...
non-reinforced demonstrative\is{demonstratives} to its left and a corresponding reinforced form to
its right:
...
In \tabref{tab:key:08.2} I summarize the various formal and functional
characteristics of the thirteen demonstrative\is{demonstratives} systems reviewed above.

\begin{table}[htpb]
\caption{Overview of \ili{Romance} demonstrative\is{demonstratives} systems}\label{tab:key:08.2}
{\smaller
...
interpreted as the (\glsunset{PF}\gls{PF}\nobreakdash-)lexicalization of specific formal feature
...
\citealt{Kayne:2000a}; \citealt{Kayne:2005a}; \citealt{Kayne:2005ab};
...
in this direction has recently been made by the \enquote{Rethinking Comparative
Syntax} (\emph{ReCoS}) research group; their central idea is that
...
[+finite]), a case of mesoparametric variation; \REF{ex:key:08.2} a small,
...
proper; and \REF{ex:key:08.3} one or more individual lexical items, a case of
...
represented schematically in \figref{fig:key:2}, which makes specific
...
specifications in \figref{fig:key:3} where projection of the
...
parameter hierarchy\is{parameter hierarchies} in \figref{fig:key:4}.

\begin{figure}[htpb]
    \caption{Parametric hierarchy for \ili{Romance} demonstrative\is{demonstratives}
    systems}\label{fig:key:4}
    \begin{tikzpicture}[baseline, align=left, scale=.9]
        ...
        \Tree 	[.{\textbf{Q1}\makebox[0pt][l]{ \emph{Does the system encode person
                    ...
                    [.{Yes\\\textbf{Q3} \emph{Individually (i.e.\ scattered)?} (\Cref{fig:key:3}b,c)}
                            {Yes: \textbf{T2\tss{(A/B)}}\\CIDs, \dots{}}
                            [.{No = (syncretically)\\\textbf{Q4} \emph{\textsc{Part}
                                (i.e.\ Sp+Ad)?} (cf.\ \Cref{fig:key:3}d)}
                                {Yes: \textbf{B1\tss{a}/2\tss{A--C}}\\OFr, SIDs, \dots{}}
                                [.{No\\\textbf{Q5} \emph{Sp?} (cf.\ \Cref{fig:key:3}c)}
                                    {Yes: \textbf{B3}\\LA Sp}
                                    {No (→ *1/3 vs.\ 2)}
                                ]
                            ]
...
the first question in \figref{fig:key:4} simply asks whether a given
...
\figref{fig:key:4} asks whether person is maximally encoded such that all
...
P\textsc{art} nodes (cf.\ options b,c in \figref{fig:key:3}) or no
...
concomitant placement towards the end of the hierarchy in \figref{fig:key:4}.
...
inclusive forms (cf.\ option d in \figref{fig:key:3}) found in
...
the bottom of hierarchy in \figref{fig:key:4}, this latter possibility
...
the hierarchy in \figref{fig:key:4} which no doubt represents a no choice
...
\figref{fig:key:3}) that formally excludes reference to the Ad.
...
and \isi{person features} we ask whether at the very least encoding of person
...
person value (cf.\ option a in \figref{fig:key:3}). In reality, this question
...
the parametric representation in \figref{fig:key:4}. First, despite my
identification of 13 formal systems in \tabref{tab:key:08.2}, the hierarchy in
\figref{fig:key:4} reduces this superficial variation in demonstrative\is{demonstratives} systems
...
Third, the representation in \figref{fig:key:4} reveals how a formal analysis
...
\figref{fig:key:4} is the conclusion that the T1 systems observed above in
