...
\section{Introduction}\label{sec:key:06.1}
...
varieties.\footnote{Though our focus is on Northern \ili{Italian} dialects
    (\Cref{sec:key:06.2}) and on standard \ili{Italian} (\Cref{sec:key:06.1}), the
    title refers to \ili{Romance} varieties, in that the database of
\citet{ManziniSavoia2005}, which we use in particular in \Cref{sec:key:06.2},
includes \ili{Occitan}, Franco-Provençal and \ili{Ladin} (Rhaeto-Romance) dialects, spoken
within the borders of Italy and Switzerland.} In \Cref{sec:key:06.2} we argue
...
is as in \tabref{tab:key:06.1}. What is immediately evident from the table is that
...
\begin{table}
    ...
    \begin{tabularx}{\textwidth}{XXXXXX}
                 & \Acc.\M & \Acc.\glossF{} &           & \Dat.\M     & \Dat.\glossF{}\\
    \lsptoprule
    \Fsg       &           &           & \emph{mi} &               & \\
    \Ssg       &           &           & \emph{ti} &               & \\
    ...
    \lspbottomrule
    \end{tabularx}\caption{Italian accusative\is{accusative case} and dative\is{dative case}
    clitics}\label{tab:key:06.1}
\end{table}
\newpage
The classical approach to asymmetries like those in \REF{ex:key:06.1} is to
...
are illustrated in \REF{ex:key:06.1}.

\ea\label{ex:key:06.1} \ili{Italian}
    \ea[]{
...
not \isi{Agree} in either gen\-der or number, as in (\ref{ex:key:06.2}a),
paralleling the dative\is{dative case} clitic in (\ref{ex:key:06.2}c).
Agreement of the 1/2P clitic with the perfect participle, as seen in
(\ref{ex:key:06.2}b), remains possible, but it is optional. Free alternations
...
non-agreeing participles in \REF{ex:key:06.3}, we expect there to be languages
...
\ea\label{ex:key:06.2} \ili{Italian}
   	\ea
...
clearly obligatory in subject contexts, as in \REF{ex:key:06.3}. The same
...
The alternative option, taken by \citet{ManziniSavoia2005}, \citet{Kayne2010},
...
\citet{ManziniSavoia2005}, \citet{Manzini2012}, \citet{ManFra2016}
...
\subsection{Clitics and Case}\label{sec:key:06.1.1}
...
singular 3P \isi{clitics}, this yields structures like \REF{ex:key:06.4}.
...
Languages like \ili{Spanish} have an independent lexicalization for the plural,
name\-ly \emph{-s}; in \ili{Italian} however pluralization is obtained by a change of
the inflectional vowel. We may suppose that the plural 3P \isi{clitics}, namely
\emph{li/le}, have the structure in \REF{ex:key:06.5}, where the plural property
...
The morphological structures in \REF{ex:key:06.4}--\eqref{ex:key:06.5} map to a
...
feature set as descriptive features, which are nominal, hence Ns”. In this
...
(morpho)syntactic generalization – namely that 1/2P are pure deictic forms,
...
\ili{Greek}). In fact, in \ili{Italian} \REF{ex:key:06.2}, the \emph{m-i,} \emph{t-i} 1/2P
person forms have the same \emph{-i} inflection as the 3P dative\is{dative case} \emph{gl-i}.
This inflection contrasts with that of the accusative\is{accusative case} in \REF{ex:key:06.1},
corresponding to gender morphology (\emph{-o, -a, -i, -e}).\footnote{\emph{-i}
    ...
    further that though in \tabref{tab:key:06.1}, we have illustrated normative
...
According to Dixon, “it is plainly most natural and economical to “mark” a
...
touches what we may call split accusativity.” Similarly, using a different
...
“Entities have various zones associated with them, such that an object or
...
In these terms, the structure of embedding of \emph{mi/ti} in \REF{ex:key:06.2}
...
(to)’ with 3P \isi{clitics} in \REF{ex:key:06.1}. In the structure in \REF{ex:key:06.6} we
...
what we mean when we say that in \REF{ex:key:06.6}, ${\subseteq}$ takes as its
arguments the 1/2P pronoun and an elementary state/event. In other words,
\REF{ex:key:06.6} can be informally rendered as ‘He caused me to have help/talk’.
We claim that the 1/2P pronoun in \REF{ex:key:06.6} is introduced as a possessor,
...
(to)’ does not, as in (\ref{ex:key:06.7}a) vs.\ (\ref{ex:key:06.7}b). The contrast in
...
(\ref{ex:key:06.7}b$'$) but barring \isi{raising} to nominative\is{nominative case}
position as in (\ref{ex:key:06.7}b). On the contrary, the dative\is{dative case} case with
...
need be present in the derivation and sentences like (\ref{ex:key:06.7}a) are
well-formed.
...
varieties only 1/2P internal arguments require DOM, as in (\ref{ex:key:06.8}a).
3P pronouns and kinship terms (essentially functioning as proper names) undergo
ordinary (bare) embedding, as in (\ref{ex:key:06.8}b).\footnote{Other varieties
...
\ea%8
    \label{ex:key:06.8} \ili{Italian} Colledimacine (Abruzzi, \citealt{ManziniSavoia2005}: 505)
    \ea
...
no question that facts like \REF{ex:key:06.8} are syntactic.

\subsection{Clitics and Agree}\label{sec:key:06.1.2}
...
(\ref{ex:key:06.9}a); for the sake of exposition we have assumed that the
...
(\ref{ex:key:06.9}b). The traditional assumption in this respect is that some
...
\REF{ex:key:06.10}. The φ constituent is presumably to be identified
...
Italian \REF{ex:key:06.9}, will be construed as an elementary lexicalization of
the internal argument within the morphological structure of the verb, as
schematized in \REF{ex:key:06.10}. Classical theories of pro-drop\is{null subjects} hold the view
...
In \REF{ex:key:06.11}, the φ constituent endowed with gender and number
...
for the probe--goal system” are formulated as in \REF{ex:key:06.12}. Matching,
namely feature identity according to (\ref{ex:key:06.12}a), “is a relation that
...
(\ref{ex:key:06.12}b).  Furthermore, “a matching feature G is closest to P if there
is no G' in D(P) matching P such that G is in D(G')” as in (\ref{ex:key:06.12}c).

\ea%12
    \label{ex:key:06.12} \textcite[122]{Chomsky2000}
...
Specifically, we may expand the schematic structure in (\ref{ex:key:06.9}a) as in
\REF{ex:key:06.13}. We translate the classical idea that φ features\is{φ-features} percolate to
...
\REF{ex:key:06.12} creating a pair ordered by c-command and obeying locality,
...
agreement pair for \REF{ex:key:06.13} unifies the feminine features instantiated
...
Let us then consider the 3P non-agreeing pattern in (\ref{ex:key:06.9}b). The
internal structure of the perfect participle is as already indicated in
\REF{ex:key:06.9}, except that \emph{parlare} ‘speak (to)’ does not introduce an
...
ending, as in \REF{ex:key:06.14}; the latter could be the realization of an empty
...
\REF{ex:key:06.14-2}. The φ probe can be matched with the 1/2P content as
...
\REF{ex:key:06.13} that the right way to think about agreement pairs is not in
...
Next, consider the non-agreeing 1/2P structure in \REF{ex:key:06.15}. With
\emph{parlare} ‘talk (to)’, as already reviewed in relation to the 3P clitic in
\REF{ex:key:06.13}, ${\subseteq}$ is selected by the verb, and an agreement probe
...
probe may be generated and satisfied along the lines of \REF{ex:key:06.14-2}. We now
...
(non-selected) one as in~\eqref{ex:key:06.16}.
...
\emph{ne} genitive\is{genitive case} clitic in \REF{ex:key:06.16} licences agreement in the plural
...
between a D(P) and a modifier it embeds. In \REF{ex:key:06.16} the genitive\is{genitive case}
...
\REF{ex:key:06.14} or \REF{ex:key:06.14-2}.

\ea%16
    \label{ex:key:06.16} \ili{Italian}\\
...
Participant/non-Participant Person split and specifically its interaction with
...
argument, for instance in \ili{Hindi} (\ref{ex:key:06.17}a), where the internal argument
...
as in (\ref{ex:key:06.17}b). What is relevant here is that the \gls{DOM}\is{differential object marking} object does not
\isi{Agree} with the perfect participle, which shows up in the default
masculine singular.

\ea%17
    \label{ex:key:06.17} \ili{Hindi} \parencites[83]{Mohanan1995}[3]{Ahmed2006}
  	\ea
...
question” \parencite[234]{Verbeke2013}. In \REF{ex:key:06.18} we illustrate just
agreement of the perfect participle with \gls{DOM}\is{differential object marking} objects (-\emph{nai}).

\ea%18
    \label{ex:key:06.18}\ili{Rajasthani} \citep{Khokhlova2002}\\
...
correct.\footnote{We do not have data on how \gls{DOM}\is{differential object marking} interacts with perfect
    participle agreement in varieties like Colledimacine in \REF{ex:key:06.8}
...
\section{1P vs.\ 2P\@: Northern \ili{Italian} subject clitics}\label{sec:key:06.2}
...
going back to \Cref{sec:key:06.1}, 1/2P vs.\ 3P.

\subsection{Partial pro-drop\is{null subjects} in Northern \ili{Italian}
dialects}\label{sec:key:06.2.1}
...
\REF{ex:key:06.19}. This result remains constant if instead of considering null
subjects slots, we consider slots taken by syncretic \isi{clitics} lacking
specialized P morphology.

\ea\label{ex:key:06.19}\leavevmode\\[-1\baselineskip]%
...
other existing languages of table \REF{ex:key:06.19} externalize subject \isi{clitics}
...
\REF{ex:key:06.20}, in terms of the salience of speaker reference.
...
\eqref{ex:key:06.20}, interacting with a universal rule/principle of grammar,
namely Recoverability \REF{ex:key:06.21}, explains why Càsola in line 3 of table
\REF{ex:key:06.19} is a possible language, while its mirror image in line 8 is
...
enrichment of L(ogical)F(orm), as in \REF{ex:key:06.21}; in either case its
content remains constant, i.e.\ that of licensing lack of Externalization. The
salience of 1P in \REF{ex:key:06.20} makes it (pragmatically) recoverable, in the
sense of \REF{ex:key:06.21}, independently of any other syntactic or semantic
condition being satisfied – licensing its lack of externalization. This is not
the case for 2P, which must therefore be lexicalized. Therefore \REF{ex:key:06.20}
crossed with Recoverability yields the prevalence of 2P lexicalizations over 1P
ones in table \REF{ex:key:06.19}. To be more precise, rows 1-3 are allowed because
...
\ea%21
    \label{ex:key:06.21} Recoverability\\
    Recover non-externalized LF content (referential etc.)
\z

Nevertheless, there are patterns in table \REF{ex:key:06.19} which are excluded
...
plural but not in the singular. We may therefore assume that \REF{ex:key:06.20}
either applies to the singular, i.e.\ to speaker proper, or it cannot apply at
all, as in \REF{ex:key:06.22}. In other words, it is possible for it to be defined
in the singular of a given language, and not in the plural– but not vice versa.
A point to which we will return is that \REF{ex:key:06.22} is a statement about a
...
\ea%22
    \label{ex:key:06.22}
    \REF{ex:key:06.20} is not defined in the plural.
\z

Recall next that table \REF{ex:key:06.19} records the attested variation in P
...
to define an independent parameter. Thus in table \REF{ex:key:06.19} there are
...
\REF{ex:key:06.23}, i.e.\ the 1/2P vs.\ 3P split also dealt with in
\Cref{sec:key:06.1}.
...
One may then expect the reverse situation to \REF{ex:key:06.19} to be attested,
...
P slots vary along the lines discussed for table \REF{ex:key:06.19} – i.e.
...
facts are depicted in table \REF{ex:key:06.24}, where pattern numbers refer
back to corresponding patterns in \REF{ex:key:06.19}. Evidently, our analysis
...
\ea\label{ex:key:06.24} \leavevmode\\[-1\baselineskip]%
\begin{tabular}{llllllll}
    &                  & 1st & 2nd & 3rd & 4th & 5th & 6th \\
1.  & *                & --- & P   & --- & P   & P   & --- \\
2.  & \emph{Tetti}     & --- & P   & --- & --- & --- & --- \\
...
proposal we put forth in \REF{ex:key:06.22}, saying that the 1P vs.\ 2P split may
...
plural, along the lines in \eqref{ex:key:06.22prime}, but not vice versa. Thus, since 3P singular
will have nominal class properties, along the lines of \Cref{sec:key:06.1}, we
...
configurations in table \REF{ex:key:06.19}, we may expect six patterns, as in
\REF{ex:key:06.25}. Only two of them are found, namely pattern 13, where only the
...
\ea\label{ex:key:06.25} \leavevmode\\[-1\baselineskip]%
    \begin{tabular}{llllllll}
    &                  & 1st & 2nd & 3rd & 4th & 5th & 6th \\
1.  & *                & --- & P   & D   & P   & P   & --- \\
2.  & \emph{Olivetta}  & --- & P   & D   & --- & --- & --- \\
...
\REF{ex:key:06.26}. Parameter values are the \gls{SM} choices that \REF{ex:key:06.26}
...
\ea%26
    \label{ex:key:06.26} Externalization\\
...
parameter. Generalizing from statements like \REF{ex:key:06.22},
\eqref{ex:key:06.22prime} one may
further surmise a schema for the interaction between \isi{parameters}, as in
\REF{ex:key:06.27}. In other words, when \isi{parameters} cross, one of them may remain
...
parameter proposed within the Rethinking Comparative Syntax (ReCoS) project.
...
\subsection{Competing views of parametrization}\label{sec:key:06.2.2}

The data tabulated in \REF{ex:key:06.19} have attracted at least two types of
...
configuration in \REF{ex:key:06.28}.  They propose that in \REF{ex:key:06.28}
...
varieties like Livo in table \REF{ex:key:06.19}, 3P subject \isi{clitics} are realized,
...
Recall that in introducing table \REF{ex:key:06.19} we have noticed that the
...
in \REF{ex:key:06.29}. For instance, the activation of constraint (\ref{ex:key:06.29}f)
...
\NumTabs{5}
\ea%29
    \label{ex:key:06.29} In the context [[AgrS \underline{\hspace{2em}} ] +V
...
lexicalized, like Livo in table \REF{ex:key:06.19}. Furthermore, Calabrese also
...
specialized – such as Prali in table \REF{ex:key:06.19}. From a theoretical point
...
Technically, in his filter hierarchy in \REF{ex:key:06.29}, the more marked a form
...
\eqref{ex:key:06.20}, line 1; the approach in \citeauthor{CardinalettiRepetti2008} does not
...
\REF{ex:key:06.23} is independent of the 1P vs.\ 2P split in \REF{ex:key:06.20}
...
patterns in table \REF{ex:key:06.19} are instantiated
...
magnitude of overgeneration is 4 patterns over 64 (6\textsuperscript{2}),
namely one in table \REF{ex:key:06.25} and three in table \REF{ex:key:06.24}.
...
\ili{Italian} varieties, have this to say: “The distinction between
...
a single lexical item qualifies it as microparametric”. Transposing this
discussion to the case study in \Cref{sec:key:06.2.1}, Speaker, Plural,
...
\isi{microvariation} in subject clitic systems in table \REF{ex:key:06.19}.
Going back to \Cref{sec:key:06.1}, the same holds for DOM, which may determine
...
which we reproduce in \REF{ex:key:06.30}.

\ea\label{ex:key:06.30} Null arguments hierarchy\\*[-1\baselineskip]
    \hspace*{-.75cm}%
...
The macroparametric region of the schema in \REF{ex:key:06.30} corresponds to
(\ref{ex:key:06.30}a)--(\ref{ex:key:06.30}c). In (\ref{ex:key:06.30}a), lack of
...
namely radical pro-drop\is{null subjects} languages (languages of the Chinese/Japanese type). In
(\ref{ex:key:06.30}b), the universal value of the parameter, corresponding to
...
(\ref{ex:key:06.30}c), which posits the existence of uninterpretable
...
with all T heads (\ref{ex:key:06.30}d), and presumably further down with some T
...
instance, in the macroparametric steps (\ref{ex:key:06.30}a--c), the
uninterpretable phi-features\is{φ-features} property is evaluated in relation to functional
heads, while in the mesoparametric steps from (\ref{ex:key:06.30}d) down it is
...
from (\ref{ex:key:06.30}b) to (\ref{ex:key:06.30}c), the query switches from
...
is devoted to the progression down such hierarchies, like \REF{ex:key:06.30}.
...
hierarchy in \REF{ex:key:06.30}, i.e.\ Chinese-style “radical pro-drop”
...
\section{Conclusions}\label{sec:key:06.3}
