...
structure constraint} is standardly assumed to have two parts, given in \eqref{ex-26:key:1} and
...
he notes that \ili{Japanese} observes \eqref{ex-26:key:1}, but not \eqref{ex-26:key:2}, allowing extraction of
...
\gls{CSC}\is{coordinate structure constraint} to refer only to \eqref{ex-26:key:1}. (Where it
...
\ea[*]{%
    ...
    t\tss{i}] and [a painting of Storrs]]?}\label{ex-26:key:7}
\z

\ea[?*]{%
    What\tss{i} did you [\tss{VP} [\tss{VP} fall
    asleep] [after John had fixed t\tss{i}]]?}\label{ex-26:key:8}
\z
...
out extraction out of adjuncts\is{adjunction}.\footnote{There is an important issue that
    ...
    which is however based on the assumption that conjuncts and adjuncts\is{adjunction} have a
different syntax).}
...
the \gls{CSC}\is{coordinate structure constraint} and the \glsdesc{AC}\is{adjunct condition}. §4 discusses and resolves some potential
impediments to the unification of the islandhood\is{islands} of conjuncts and adjuncts\is{adjunction}. §5
discusses extraction of conjuncts and adjuncts\is{adjunction}. §6 concludes the paper.

\section{The stubbornness of the \glsentrytext{CSC} and the \glsentrytext{AC}}
...
\section{Some exceptions to the \glsentrytext{CSC} and the \glsentrytext{AC}}
...
\ea[*]{How\tss{i} should athletes [ take that drug t\tss{i} ] and
become strong?}
\z
...
\subsection{Across-the-board \isi{movement} and parasitic gaps}\is{parasitic gaps}
...
\ea\label{ex:key:35.17}
    ...
    \ex\label{ex-26:key:17c} \glsdesc{SC}\il{Serbo-Croatian} \parencites[31]{Despic2011}[245]{Despic2013}\\
...
issue in (\ref{ex:key:35.17}c) and Condition C in (\ref{ex:key:35.17}d)), which contrast with \ili{English}
...
\ea\label{ex-26:key:18}\glsdesc{SC}\il{Serbo-Croatian}\\
    \gll Trčao  je  šumom.\\
            run is forest.\Ins{}\\
...
\ea\label{ex-26:key:19} \ili{Serbo-Croatian}\\
    \ea
        \gll    \llap{*}Šumom\tss{i} se pitaš [ kad je trčao t\tss{i} ].\\
                forest.\Ins{} \Refl{} wonder {} when is run\\
        \glt    \enquote*{You wonder when he ran through a/the forest.}
	\ex
        \gll    \llap{??}Šumu\tss{i} se pitaš [ kad je posjekao t\tss{i} ].\\
...
degraded in \glsentrytext{SC}, (\ref{ex-26:key:20}a), which involves extraction
...
\ea\label{ex-26:key:20} \glsdesc{SC}\il{Serbo-Croatian}\\
    \ea
        \gll    \llap{*}Moga djeda\tss{i} je trčao [ šumom t\tss{i} ].\\
                my.\Gen{} grandfather.\Gen{} is run {} forest.\textsc{instr}\\
        \glt    \enquote*{He ran through the forest of my grandfather.}
    \ex
        \gll    \llap{??}Moga djeda\tss{i} je volio [ šumu t\tss{i} ].\\
...
\ea\label{ex-26:key:21}\glsdesc{SC}\il{Serbo-Croatian}\\
...
\eqref{ex-26:key:22}, but an adnominal genitive\is{genitive case} cannot \eqref{ex-26:key:23}.\footnote{Left-branch extractions
...
\ea\label{ex-26:key:22}\glsdesc{SC}\il{Serbo-Croatian}\\
...
\ea\label{ex-26:key:23}\glsdesc{SC}\il{Serbo-Croatian}\\
...
\glsentrytext{AC} and rescue by PF deletion}
...
in embedded clauses, even with \isi{coordination}).} Consider \eqref{ex-26:key:24}--\eqref{ex-26:key:25}, which are
intended to represent a case of traditional \isi{coordination} \eqref{ex-26:key:24} and a case of
traditional \isi{adjunction} \eqref{ex-26:key:25}, which is also treated as involving coordination
...
The conjuncts in the traditional \isi{coordination} in \eqref{ex-26:key:24} are symmetric regarding
...
\ea\label{ex-26:key:27} What\tss{i} did you [ buy t\tss{i} ] slowly?
\z
...
conjuncts are \isi{islands} (as explicitly also argued in \citealt{Odainpress}). The
...
\ea\label{ex-26:key:28} Traces do not head \isi{islands}.
\z
...
Galician disallows \isi{movement} from definite DPs, as in \eqref{ex-26:key:29}. However, the
...
of the islandhood-voiding effect of article \isi{incorporation} is provided by \eqref{ex-26:key:31}.
Extraction from adjuncts\is{adjunction} is banned in \ili{Galician}, as in \eqref{ex-26:key:31}.  However, the ban
...
\ea\label{ex-26:key:29} \ili{Galician} \parencite[81]{Uriagereka1988}
    \gll \llap{*}e de quén\tss{i} viches [\tss{DP} o [\tss{NP} retrato t\tss{i}]]?\\
        and of who saw(you) {} the {} portrait\\
\z

\ea\label{ex-26:key:30} \ili{Galician} \parencite[81]{Uriagereka1988}
...
\ea\label{ex-26:key:31} \ili{Galician} \parencite[58]{Boskovic2016}
    \gll \llap{??}de que semana\tss{j} traballastedes [\tss{DP} o Luns t\tss{j}]\\
		    of which week worked(you) {} the Monday\\
    \glt \enquote*{Of which week did you guys work the Monday?}
\z

\ea\label{ex-26:key:32} \ili{Galician} \parencite[58]{Boskovic2016}
	\gll    de  que  semana\tss{j}  traballastede-lo\tss{i} [\tss{DP} [\tss{D$'$} t\tss{i}  Luns t\tss{j}]] \\
            of  which week worked(you)-the {} {} {} Monday\\
\z

These cases illustrate the generalization in \eqref{ex-26:key:28}. The islandhood\is{islands}
...
effects are also subsumed under (\ref{ex-26:key:28}); i.e.\ they also involve \isi{islands}
...
The effect from \eqref{ex-26:key:33} is standardly treated in terms of rescue by \glsentrytext{PF} deletion
...
(\ref{ex-26:key:33}b) the islandhood\is{islands} effect disappears in this example.

\textcite{Boskovic2011,Boskovic2013b} also provides a
rescue-by-\glsentrytext{PF} deletion account of the generalization in \eqref{ex-26:key:28},
unifying \eqref{ex-26:key:28} with the rescuing effect of \isi{ellipsis} on islandhood\is{islands}.
...
offending *-marked element is thus deleted in \glsentrytext{PF} in \eqref{ex-26:key:30}, just
as it is in \eqref{ex-26:key:33}. The analysis quite generally captures the generalization in
\eqref{ex-26:key:28}.\footnote{The analysis predicts that \isi{head movement} is not sensitive to
    ...
    \ili{Galician} article \isi{incorporation} (cf.\ (\ref{ex-26:key:32})), which is also
...
is V-to-v \isi{movement}.\footnote{ There are various proposals in the literature
    ...
    element) in \eqref{ex-26:key:27} is an island. However, V-to-v \isi{movement}, i.e.\ \isi{movement} of
    ...
    given in \eqref{ex-26:key:28}. The potential obstacle to the unification of the \glsentrytext{CSC}
    and the \glsentrytext{AC}\is{adjunct condition} that was raised by \eqref{ex-26:key:27} is thus rather straightforwardly
...
the \glsentrytext{CSC} and the \glsentrytext{AC}\is{adjunct condition} raised by \eqref{ex-26:key:27} but it also makes a prediction.
Consider again \eqref{ex-26:key:24}--\eqref{ex-26:key:25}. Just like in \eqref{ex-26:key:25} \isi{movement} of the head of the VP
conjunct makes \isi{movement} out of the VP possible so should \isi{movement} of the head
of the corresponding conjunct in \eqref{ex-26:key:24} make \isi{movement} out of this conjunct
...
marker).\is{differential object marking}\is{DOM|see{differential object marking}}

\ea\label{ex-26:key:34} \ili{Galician}\\
    \gll    \llap{*}De quén\tss{i} vistedes [ o amigo t\tss{i} ] e-mais [ a Xan ] onte?\\
            of who (you)saw {} the friend {} {} and {} \Dom{} Xan {} yesterday\\
    \glt    intended: \enquote*{You saw [[the friend of who] and [Juan]] yesterday?}
\z

\ea\label{ex-26:key:35} \ili{Galician}\\
    \gll \llap{??}De quén\tss{i} vistede-lo\tss{j} [ t\tss{j} amigo t\tss{i} ] e-mais [ a Xan ] onte?\\
        of who (you)saw-the {} {} friend {} {} and {} \Dom{} Xan {} yesterday\\
\z

\eqref{ex-26:key:34} shows that extraction out of a conjunct is not possible in \ili{Galician}, i.e.\
conjuncts are \isi{islands}. Importantly, \eqref{ex-26:key:35}, which involves article \isi{incorporation}
from the conjunct from which \emph{wh}-movement takes place, is clearly better
than \eqref{ex-26:key:34}, which does not involve article \isi{incorporation}. Article \isi{incorporation}
thus also improves extraction out of conjuncts.

Putting for the moment the residual awkwardness of \eqref{ex-26:key:35} aside, and focusing on
the fact that \eqref{ex-26:key:35} is better than \eqref{ex-26:key:34}, the current analysis unifies the
grammaticality of \eqref{ex-26:key:27} with the improvements that article \isi{incorporation} causes
for \emph{wh}-movement in \eqref{ex-26:key:31}--\eqref{ex-26:key:32} and \eqref{ex-26:key:34}--\eqref{ex-26:key:35}. All the relevant cases
...
parts of the \glsentrytext{CSC}, i.e.\ \eqref{ex-26:key:1}--\eqref{ex-26:key:2}, by proposing that both individual
...
In \eqref{ex-26:key:27} and \eqref{ex-26:key:32}, both the islandhood\is{islands} of the relevant individual conjuncts and
...
\eqref{ex-26:key:34} in a sense involves two violations, since the heads of both \isi{islands}, the
relevant conjunct and ConjP, are phonologically overt. On the other hand, in
\eqref{ex-26:key:31} only the former is phonologically overt: the islandhood\is{islands} of ConjP is voided
here since the head of ConjP itself is phonologically null. Furthermore, notice
that standard \glsentrytext{CSC} violations like (\ref{ex-26:key:26}a) are worse than traditional
...
voiding the islandhood\is{islands} of the conjunct. The grammaticality of \eqref{ex-26:key:27} then turns
...
the \enquote{host} of \isi{adjunction}, i.e.\ the VP in \eqref{ex-26:key:25}, but also for the
traditional adjunct itself.  As shown in \eqref{ex-26:key:31}--\eqref{ex-26:key:32}, the islandhood\is{islands}
...
can be added to the cases discussed in §3: both the \glsentrytext{CSC} and the
\glsentrytext{AC}\is{adjunct condition} effect are voided under \isi{head
...
\eqref{ex-26:key:24}--\eqref{ex-26:key:25}, we have seen that \isi{head movement} rescues extraction out of both
conjuncts in the traditional \isi{adjunction} case in \eqref{ex-26:key:25}, i.e.\ it makes extraction
...
out of the second conjunct in \eqref{ex-26:key:24}. Does \isi{head movement} of the head of that
...
conjunct in \eqref{ex-26:key:24} with article \isi{incorporation} in \ili{Galician}. Does the effect also
...
\emph{wh}-extraction takes place, undergoes \isi{incorporation} only in \eqref{ex-26:key:39}. (Not
...
\ea\label{ex-26:key:38}Galician
    \gll \llap{*}De qué cidade\tss{i} vistedes um retrato de Diego e mais [ a  paisaxe t\tss{i}]?\\
    of what city (you)saw a portrait of Diego and {} {} the landscape\\
\z

\ea\label{ex-26:key:39}Galician
    \gll \llap{???}De qué cidade\tss{i} vistedes um retrato de Diego e-mai-la\tss{j} [t\tss{j} paisaxe t\tss{i}]?\\
    of what city (you)saw a portrait of Diego and-the {}  landscape\\
\z
...
it does not induce a \glsentrytext{PF} violation) but that *-marked heads are unable to send
...
the *-marked null D in \eqref{ex-26:key:40} would not be able to send its complement to
spell-out.\footnote{I assume that spell-out must take place for each phasal
    ...
    there is still a difference here with the \ili{Galician} case in \eqref{ex-26:key:30}, where the
    *-marked element in D is deleted under \isi{copy deletion}. Under the analysis
    under consideration, the spell-out for the DP phase\is{phases} in \eqref{ex-26:key:30} would be
...
lexicalized with traditional \isi{adjunction} is the Avoid Overt Conjunction
Principle, which works in a similar way as \citegen{Chomsky1981} \isi{Avoid Pronoun
Principle}. We can then assume that in the relevant situations (see §6 for why
this happens with traditional \isi{adjunction}), the feature matrix of the
conjunction head (or the pronoun in the cases where the \isi{Avoid Pronoun
Principle} is relevant, see \citealt{Holmberg2005}) is deleted, as a
...
difference between the article and its trace in \ili{Galician} examples like
\eqref{ex-26:key:29}--\eqref{ex-26:key:30}: In all these cases the relevant head is *-marked due to extraction
out of a conjunct, conjuncts being \isi{islands}.  The *-marked head is then deleted
in \eqref{ex-26:key:30} (due to copy deletion) and \eqref{ex-26:key:27} (due to the Avoid Overt Conjunction
Principle, which works on a par with the \isi{Avoid Pronoun Principle}).  On
...
realized (i.e.\ pronounced) in \glsentrytext{PF} would not actually be
...
\section{On extraction of conjuncts/adjuncts}

\glsunset{PF}
As noted at the outset, the discussion in this paper is limited to islandhood
of conjuncts and adjuncts\is{adjunction}, i.e.\ extraction out of conjuncts/adjuncts; it does
not deal with extraction of conjuncts/adjuncts. As discussed in §1, while the
\gls{CSC} was traditionally assumed to hold both for extraction out of
conjuncts and for extraction of conjuncts, this view is quite clearly wrong,
since there are languages that productively allow extraction of conjuncts but
still disallow extraction out of conjuncts. This is the reason why I have put
the discussion of extraction of conjuncts, i.e.\ \eqref{ex-26:key:2}, aside above. In this
section, I will, however, make some brief remarks on extraction of conjuncts,
i.e.\ the status of \eqref{ex-26:key:2}, the reason being that the rescue-by-\gls{PF} deletion
mechanism, which I have appealed to above, turns out to be relevant to \eqref{ex-26:key:2}, as
was in fact explicitly argued in \textcite{Stjepanovic2014} and
\textcite{Odainpress}.

Notice first that the \gls{CSC}\is{coordinate structure constraint} is not
completely divorced from the \gls{AC} even when it comes to \eqref{ex-26:key:2}, i.e.\
...
Recall that \textcite{Odainpress} argues that both individual conjuncts and
ConjP are \isi{islands}. When it comes to extraction of conjuncts themselves, i.e.\
\eqref{ex-26:key:2}, what is relevant is the islandhood\is{islands} of ConjP: the island that is crossed
...
the conjunction head is a clitic,\is{clitics} which \citeauthor{Odainpress} and
\citeauthor{Stjepanovic2014} argue undergoes \isi{movement}. In \ili{Japanese}, the
conjunction is an enclitic\is{clitics} and in \gls{SC} it is a proclitic.\is{clitics} In \ili{Japanese} \eqref{ex-26:key:41},
...
\ea\label{ex-26:key:41} \ili{Japanese} \parencite{Odainpress}
    \ea[?]{
...
extraction of a conjunct is possible the conjunction head is a clitic\is{clitics} that
undergoes \isi{movement}.\footnote{As discussed in \textcite{Stjepanovic2014}, in
    \gls{SC} the conjunction procliticizes\is{clitics} to the second conjunct, which makes
    \isi{movement} of the first conjunct, as in (i), possible. (See
    \citealt{Stjepanovic2014} for details of the derivation, which also
    involves ConjP-internal \isi{movement} of the second conjunct prior to the
    ...
    proclitics;\is{clitics} thus, she shows, following \citealt{Boskovic2013b} and
    \citealt{Talic2014}, that the proclitic\is{clitics} preposition in (ii) procliticizes
    to the AP (and is carried along under further \isi{movement} of the AP, as in
    (iii)), with \citegen{Talic2014} prosodic arguments for procliticization\is{clitics} in
    terms of syntactic \isi{movement} of the preposition in (ii) extending to the
    conjunction in (i).)
...
\isi{copy deletion}). The analysis thus unifies acceptable CSC-2\is{coordinate structure constraint} violations like \eqref{ex-26:key:41}
...
adjunct) in \eqref{ex-26:key:42} is then able to undergo \isi{movement} is the same as the reason why
...
and ConjP are \isi{islands}. In cases like \ili{Galician} \eqref{ex-26:key:34}, both of these \isi{islands} are
“violated”. In \eqref{ex-26:key:35}, on other hand, the islandhood\is{islands} of the conjunct island is
...
of a conjunct in \ili{English} and \ili{Galician} \eqref{ex-26:key:34} – it should be more on a par with
Galician \eqref{ex-26:key:35} than \ili{Galician} \eqref{ex-26:key:34}. The prediction is in fact more general, it
...
CSC-1 violations and the \isi{adjunct condition} violation. Consider cases where no
islandhood is voided through \isi{movement} of island heads (cf.\ \eqref{ex-26:key:28}). As discussed
above, both conjuncts and ConjP are \isi{islands}. Extraction out of a conjunct then
involves two island violations. Since adjuncts\is{adjunction} are treated as conjuncts,
extraction out of an adjunct also involves extraction out of a conjunct island
and a ConjP island. However, since with adjuncts\is{adjunction} the head of ConjP is
phonologically null, the islandhood\is{islands} effect of ConjP is voided, as discussed
above. Extraction out of an adjunct then involves one island violation. We may
then expect that CSC-1\is{coordinate structure constraint} violations should be stronger than adjunct condition
violations in a language like \ili{English}. That indeed seems to be the case: CSC-1
violations like \eqref{ex-26:key:4} seem to be worse than \isi{adjunct condition} violations like
\eqref{ex-26:key:5} (as noted above, the prediction is also borne out with \ili{Galician} \eqref{ex-26:key:31} and \eqref{ex-26:key:34},
\eqref{ex-26:key:34} being worse than \eqref{ex-26:key:31}). On the other hand, in a language like \gls{SC}
where the head of ConjP is also phonologically null due to the cliticization\is{clitics} of
the conjunction, extraction out of both conjuncts and adjuncts\is{adjunction} involves
extraction out of a single island. CSC-1\is{coordinate structure constraint} violations and the adjunct condition
...
careful data elicitation.\is{CSC|see{coordinate structure constraint}}
...
anything else – this is the null Conj\textsuperscript{0}.\footnote{This does
    ...
    relevant in the context of the current discussion is \citegen{Progovac1999}
    Economy of Pronunciation which works in a similar way as
    \citegen{Chomsky1981} \emph{\isi{Avoid Pronoun Principle}}, choosing the
