...
\abstract{This paper explores the possibility that the \gls{NTC} is eliminated
    ...
    \gls{PIC}. We review empirical evidence indicating that the (original
    formulation of the) \gls{NTC} is violated phase\is{phases}-internally, a
    possibility that does not extend to the \gls{PIC}.  In so doing, we also
    consider the weak version of the \gls{PIC} discussed in
\citet{Chomsky2016}.}
...
\glsunset{IC}\textsc{\glsdesc{IC}} (\gls{IC},
\citealt[228]{Chomsky1995}), the \glsunset{NTC}\textsc{\glsdesc{NTC}}
(\gls{NTC}, \citealt[138]{Chomsky2008}), and the
\glsunset{PIC}\textsc{\glsdesc{PIC}} (\gls{PIC},
\citealt{Chomsky2000}). Putting details aside, \gls{IC}, \gls{NTC}\is{No Tampering Condition} and
...
There is a very close relationship between \isi{Merge} and the \gls{NTC}\is{No Tampering Condition} on the one
...
and \isi{Agree})\footnote{I assume that \isi{Agree} actually implies a complex set of
    ...
    applied to heads, as in \citeapos{Chomsky2015} analysis of
...
\gls{FI}, with interesting consequences for \citeapos{Chomsky2015}  analysis of
the \glsunset{EPP}\gls{EPP},\is{Extended Projection Principle} discussed in \citet{Gallego2017}.} apply at the
phase level, \isi{Merge} and the \gls{NTC}\is{No Tampering Condition} do not invariably so
...
(cf.~\citealt{Chomsky1956,Uriagereka2008}):\footnote{It is typically assumed
    that all operations but \glsunset{EM}\gls{EM} apply at the phase\is{phases} level,
    simultaneously
...
rules is ordered, as in \citet{Chomsky2015}.}\is{NTC|see{No Tampering Condition}}

\ea%3
    \label{ex:key:34.3}
    \ea \gls{EM} = context-free
    \ex \glsunset{IM}\gls{IM} / \isi{Agree} / Transfer = (mildly) context-sensitive
    \z
\z
...
the \gls{EC} whereas (4b) satisfies Local \isi{Merge}. In the same breath, he
notes that \blockquote[{\citealt[136]{Chomsky2000}}][.]{weaker assumptions
    suffice to bar [(\ref{ex:key:34.4}a)] but still allow Local \isi{Merge}
...
\eqref{ex:key:34.5} holds in general, except for head \isi{adjunction}. In the case of XP merger,
...
indices, etc. can be added to them by \isi{Merge}. \citet{Chomsky2007} gives another
twist by noting that while \isi{Merge} cannot modify α or β, some subsequent
operation might:
...
After \REF{ex:key:34.8}, the workspace contains γ and nothing else, so α and β are
...
the empirical problems in \eqref{ex:key:34.10}:\footnote{If the \gls{NTC}\is{No Tampering Condition} is restricted to
...
We have seen that the \gls{NTC}\is{No Tampering Condition} has two formulations, strong and weak. Let me
...
between \glsunset{NS}\gls{NS} and both interfaces), which is defined in \eqref{ex:key:34.12}:
...
existence of structures, in \ili{Icelandic} or \ili{Spanish}, like those in \eqref{ex:key:34.13}, where T
...
\blockquote[{\citealt[143]{Chomsky2008}, my emphasis}][.]{For minimal
    ...
    a \gls{PIC}, conforming as closely as possible to \gls{SMT}\is{Strong Minimalist Thesis} [\dots{}]
    \emph{Note that for narrow syntax, probe into an earlier phase\is{phases} will
    almost always be blocked by intervention effects}. One illustration to the
    contrary is \isi{agreement} into a lower phase\is{phases} without intervention in
    experiencer constructions in which the subject is raised (voiding the
    intervention effect) and \isi{agreement} holds with the nominative\is{nominative case} object of the
    lower phase\is{phases} (Icelandic). \emph{It may be, then, that \gls{PIC} holds only
...
distinction. Take \eqref{ex:key:34.17}, call it K, where the lower phase\is{phases} complement containing
...
\ea%21
    \label{ex:key:34.21}
    \ea Do we need both \gls{NTC}\is{No Tampering Condition} and the \gls{PIC}?
	\ex If we need the \gls{PIC}, do we need the PIC3?
	\z
\z

Both \gls{NTC}\is{No Tampering Condition} and \gls{PIC} express an efficiency desideratum, namely that a
...
out.\footnote{A reviewer does not see the redundancy, as (s)he takes the
    \gls{NTC}\is{No Tampering Condition} to be a third-factor condition on \isi{Merge} (defining a Merge-cycle
...
principle. If so, then the derivation can allow tampering up to the phase\is{phases}
...
Suppose next that we apply \gls{IM} to β. If the \gls{NTC}\is{No Tampering Condition} does not hold, this
...
Recall that there are two empirical arguments to sustain it. The \isi{agreement}
facts (cf.~\eqref{ex:key:34.16}) could be tackled if \isi{Agree} takes place at the border of
\gls{NS}-externalization, not in \gls{NS}. This would have two welcome
consequences. On the one hand, we could explain the parametric nature of \isi{Agree},
which I would like to relate to \citegen{Chomsky2014} \emph{Thesis T}:
...
(\citeaposalt{Chomsky2001} Uniformity Principle;
...
given the relevance of phase-level\is{phases} memory to distinguish trivial / non-trivial
...
\citeapos{Obata2010} data are different. Consider \eqref{ex:key:34.26}:
...
\citep{Chomsky2004}, but we want α (the phase\is{phases} edge), and α alone, to remain in
