...
nouns in English, reproduced in \figref{fig:key:27.1}.
...
\begin{figure}
    ...
    \begin{tikzpicture}[baseline, font=\smaller, align=left, scale=.66]
        ...
        \node (news) at (-7, 2.5) [fill=gray!20!white]
        ...
        \node (girl) at (7, 2.0) [fill=gray!20!white]
            {+1\hphantom{\hspace{1em}\enquote{information}}\\%
        ...
        \node (legend) at (0, -5.5) [color=gray!20!blue]
        ...
        \draw [clip] (-1, -1) circle (2cm);
        \draw (1, -1) [fill=gray!30!white] circle (2cm);
    ...
    \end{tikzpicture}
    \caption{\citegen{Crystal1967} criteria for nouns}\label{fig:key:27.1}
\end{figure}
...
according to the properties and classes as in \tabref{tab:key:27.1}.
...
systems in view of the properties in \tabref{tab:key:27.1}, as illustrated in
\tabref{tab:key:27.2}.
...
their complements.\footnote{Note that the first two properties \REF{ex:key:27.2}
    and \REF{ex:key:27.3} below lump together subclasses of main verbs with (some)
auxiliaries.}
...
\begin{exe}\label{ex:key:27.2}
    ...
    \end{xlist}
\end{exe}\footnotetext{As is illustrated in (\ref{ex:key:27.2}i) and
    (\ref{ex:key:27.2}a,a$'$), not all verbs can split the complex verbs in
    their complement infinitivals. Those that do are listed in
    (\ref{ex:key:27.2}ii--iii) and illustrated in (\ref{ex:key:27.2}b,c), where
    (\ref{ex:key:27.2}ii) are examples of main verbs and (\ref{ex:key:27.2}iii)
    those of auxiliaries, as seen in~\Cref{tab:key:27.3}. The phenomenon was
...
i.e., what complement VPs each verb in the list can take, cf.\ \REF{ex:key:27.12}.

\ea\label{ex:key:27.12} \ili{Hungarian}
    \ea[]{%
...
set of features, as shown in \tabref{tab:key:27.3}, in which the lack of a
...
\caption{Feature matrix for \ili{Hungarian} verbs and auxiliaries}\label{tab:key:27.3}
\end{table}
...
points at property \REF{ex:key:27.7} as the one distinguishing them from main
...
infinitival complements, and \tabref{tab:key:27.3} was set up to include verbs
...
the items listed in \tabref{tab:key:27.3}.

\section{Conclusion: Life without word classes}
