...
\abstract{%
In this paper we compare new data from \ili{Dutch} urban youth varieties to emerging
...
subject that functions as a Familiar Topic. We adopt Walkden's (2017)
...
%Dutch and Modern \ili{Germanic} V2\is{V2 word order}
%Literature on \ili{Germanic} urban varieties and V3\is{V3 word order}
%Freywald et al., Adger, Walkden, etc
%New data from \ili{Dutch} Moroccan urban varieties shows V3\is{V3 word order} too
%Blurb about relation to Ian's work?

\noindent Main clauses in Modern \ili{Dutch} are characterized by the verb-second
...
also sections \Cref{sec:lingset} and~\ref{sec:app} below).}
...
Europe.\footnote{The term \enquote{urban youth varieties} will be used for
    ...
    of these unexpected V3\is{V3 word order} or XSV\footnote{Since the preverbal constituent is
...
orders in these languages that usually exhibit the V2\is{V2 word order} constraint have been
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Describing the V3\is{V3 word order} data}
\label{sec:data}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
sentence-initial position in Standard \ili{Dutch}, but that do occur several
...
nonetheless' cannot occur in sentence-initial position in Standard \ili{Dutch}. In
...
her focus group speaking \ili{Swedish} urban varieties. Walkden (2016), based on
...
\begin{enumerate}
    ...
    \item \enquote{Fin-V2}: Frame-Setter + Topic\is{topic} + Focus\is{focus} (\ili{Old English},
        \ili{Middle Low German}, etc.)

    \item \enquote{Force-V2 System 1}: Frame-Setter + Topic/Focus (Later
        \ili{Old French}, \ili{Spanish}, etc.)

    \item \enquote{Force-V2 System 2}: Frame-Setter\tss{HT/LD} +
        Topic/Focus\is{focus}
...
\ea\label{exsbjpro4} Standard \ili{Dutch}\\
...
\ea\label{exsbjpro4-ft} Standard \ili{Dutch} -- Familiar Topic\\
...
V3\is{V3 word order} orders (cf.\ \citealt{Walkden:2017}):\\

\begin{itemize}

    \item[\textbf{Stage 1}] L2 learners of standard \ili{Germanic} V2\is{V2 word order} fail to acquire
        \isi{verb movement} to C, resulting in SVO orders

    \item[\textbf{Stage 2}] L1 learners (e.g.\ children of first-generation
    ...
    \item[\textbf{Stage 3}] V3\is{V3 word order} structures are propagated across communities and
...
\textbf{Stage 1} of the analysis hinges on the failure of the acquisition\is{language acquisition} of
...
Assuming \textbf{Stage 1} has resulted in the failed acquisition\is{language acquisition} of \is{verb movement}V-to-C
...
Recall Wolfe's typology of V2\is{V2 word order} languages from \Cref{sec:ana1}, presented here in
the original figure from \textcite[31]{Wolfe:2017}:

\begin{figure}[h]
\caption{V3* in V2\is{V2 word order} languages \parencite[31]{Wolfe:2017}}
\begin{tikzpicture}[node distance=.66cm, text width=3.5cm,
    ...
    \begin{minipage}[t][4.5cm]{3.5cm}
        \centering
        \textbf{Fin-V2 System}\\[\baselineskip]
    ...
    \begin{minipage}[t][4.5cm]{3.5cm}
        \centering
        \textbf{Force-V2 System (1)}\\[\baselineskip]
        Frame-Setter + Topic/Focus\\[\baselineskip]
    ...
    \begin{minipage}[t][4.5cm]{3.5cm}
        \centering
        \textbf{Force-V2 System (2)}\\[\baselineskip]
...
constituent was found sentence-initially, making them \enquote{Force V2\is{V2 word order} System
1} languages.  In both \ili{Germanic} and \ili{Romance}, Wolfe thus observes a change from
...
\noindent \Cref{default} shows the dates and locations of interviews in
