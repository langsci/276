...
of the indirect object, illustrated in \REF{ex:key:04.1}. The \isi{passive}
...
reported not to have a canonical \textsc{goal} passive \REF{ex:key:04.2}
...
\REF{ex:key:04.3}, is also common in the literature, as in, for instance,
...
\subsection{The data: overview}\label{sec:key:04.2.1} %2.1. /
\glsunset{WF}

As shown by the examples in \REF{ex:key:04.4} and \REF{ex:key:04.5},
...
promoted to become the subject of the \isi{passive} sentence (\ref{ex:key:04.4}b).
Similarly, the indefinite \textsc{goal} \emph{nen student} (‘a student’) in
active (\ref{ex:key:04.5}a) has been promoted to subject status in the \isi{passive}
...
\ea\label{ex:key:04.4}\glsdesc{WF}\il{West Flemish}
    \ea
...
\ea\label{ex:key:04.5}\glsdesc{WF}\il{West Flemish}
    \ea
...
(\ref{ex:key:04.5}b) is not a property specific to the \textsc{goal} \isi{passive}. The
...
or in \isi{passive} monotransitive (\ref{ex:key:04.6}b).

\ea\label{ex:key:04.6} \glsdesc{WF}\il{West Flemish}
    \ea
...
\Cref{sec:key:04.2.2} provides arguments to the effect that in
\gls{WF}\il{West Flemish} \textsc{goal} passives, the \textsc{goal} argument is
promoted to subject status. \Cref{sec:key:04.2.3} shows that
...
In the \gls{WF}\il{West Flemish} \textsc{goal} passives (\ref{ex:key:04.4}b) and
(\ref{ex:key:04.5}b), the promoted \textsc{goal} acquires the syntactic properties
of the \gls{WF}\il{West Flemish} subject, both when definite (\ref{ex:key:04.4}b)
...
Again the agreement is mandatory (\ref{ex:key:04.8}b--d). The patterns in \REF{ex:key:04.7} and
...
\ea\label{ex:key:04.7} \glsdesc{WF}\il{West Flemish}
    \ea[]{
	\gll   dan     \textbf{de} \textbf{studenten}   dienen   bureau beloofd   {wierden / woaren}\\
		  that.\Pl{}  the students   that   office  promised   were-\Pl{}\\
    \glt   ‘that the students were promised that office’}
    \ex[*]{
	\gll  dat     \textbf{de} \textbf{studenten}   dienen bureau beloofd   {wierden / woaren}\\
		  that.\Sg{}  the students   that office  promised   were-\Pl{}\\
    \glt}
    \ex[*]{
	\gll   dan     \textbf{de} \textbf{studenten}   dienen bureau beloofd   {wierd / was}\\
		  that.\Pl{}  the students   that office  promised   was-\Sg{}\\
    \glt}
    \ex[*]{
	\gll   dat     \textbf{de} \textbf{studenten}   dienen bureau beloofd   {wierd / was}\\
		  that.\Sg{}  the students   that office  promised   was-\Sg{}\\
    \glt}
    \z
\z

\ea\label{ex:key:04.8} \glsdesc{WF}\il{West Flemish}
    \ea[]{
	\gll   dan   *(der)    \textbf{drie} \textbf{studenten} dienen bureau beloof   {wierden / woaren}\\
		  that.\Pl{} \hphantom{*(}there  three students   that office  promised were-\Pl{}\\
    \glt   ‘that three students were promised that office’}
    \ex[*]{
	\gll  dat    *(der)    \textbf{drie} \textbf{studenten} dienen bureau beloofd    {wierden / woaren}\\
		  that.\Sg{} \hphantom{*(}there  three students   that office  promised  were-\Pl{}\\
    \glt}
    \ex[*]{
    \gll dan   *(der)    \textbf{drie} \textbf{studenten} dienen bureau beloofd    {wierd / was}\\
		  that.\Pl{} \hphantom{*(}there  three students   that office  promised  were-\Sg{}\\
      \glt}
    \ex[*]{
	\gll  dat   *(der)    \textbf{drie} \textbf{studenten} dienen bureau beloofd    {wierd / was}\\
		  that.\Sg{} \hphantom{*(}there  three students   that office  promised  were-\Pl{}\\
    \glt}
    \z
\z
...
pronoun doubling. In (\ref{ex:key:04.9}a) the strong nominative\is{nominative
...
\ea\label{ex:key:04.9}\glsdesc{WF}\il{West Flemish}
    \ea
...
in \gls{WF}\il{West Flemish} (\ref{ex:key:04.10}b), and they are unavailable in
object relativization (\ref{ex:key:04.10}c). See
\textcite{Haegeman1984,Haegeman1992}.

\ea\label{ex:key:04.10}\glsdesc{WF}\il{West Flemish}
    \ea
...
\ea\label{ex:key:04.11}\glsdesc{WF}\il{West Flemish}
    \ea
...
\REF{ex:key:04.12} and to the finite verb in root clauses \REF{ex:key:04.13}.
...
\textsc{goal} \emph{Valère.} In \REF{ex:key:04.13}, the same adjacency
requirement is illustrated for root clauses in which the finite verb, here the
auxiliary \emph{wierd} (‘was’), has moved to C.  \REF{ex:key:04.14} and
\REF{ex:key:04.15} show that identical adjacency restrictions apply to definite
subjects of transitive sentences.

\ea\label{ex:key:04.12}\glsdesc{WF}\il{West Flemish}
    \ea[]{
	\gll    dat (*gisteren / verzekerst) \textbf{Valère}   die posten   beloofd   wierd\\
    ...
    \ex[*]{
	\gll    dat   die posten   \textbf{Valère}   beloofd   wierd\\
		    that   those jobs   Valère     promised   was\\
    \glt}
    \z
\z\newpage

\ea\label{ex:key:04.13}\glsdesc{WF}\il{West Flemish}
    \ea[]{
	\gll   Daarom     wierd  (*gisteren / verzekerst) \textbf{Valère}   die posten   beloofd.\\
    ...
    \ex[*]{
	\gll   Daarom     wierd   die   posten   \textbf{Valère}   beloofd.\\
    {for that reason}   was   those   jobs   Valère     promised\\
       \glt}
    \z
\z

\ea\label{ex:key:04.14}\glsdesc{WF}\il{West Flemish}
    \ea[]{
	\gll   dat   (*gisteren / verzekerst)   \textbf{Valère} die posten   beloofd eet\\
		    that   \hphantom{(*}yesterday {} probably     Valère those jobs   promised has\\
    \glt     ‘that (probably) Valère promised those jobs (yesterday).’}
    \ex[*]{
	\gll   dat   die posten   \textbf{Valère}   beloofd   eet\\
...
\ea%15
    \label{ex:key:04.15}\glsdesc{WF}\il{West Flemish}
    \ea[]{
	\gll   Daarom     eet   (*gisteren / verzekerst) \textbf{Valère}   die posten   beloofd.\\
    ...
    \ex[*]{
	\gll   Daarom   eet   die posten   \textbf{Valère}   beloofd.\\
...
(\ref{ex:key:04.16}a). The goal subject of a \isi{passive} clause may undergo
raising in \emph{te} infinitives (\ref{ex:key:04.16}b).

\ea%16
    \label{ex:key:04.16}\glsdesc{WF}\il{West Flemish}
    \ea
...
\isi{passive} clause under one shared subject DP (\ref{ex:key:04.17}b).
...
\textsc{agent} can be overtly expressed \REF{ex:key:04.18}.

\ea%18
    \label{ex:key:04.18}\glsdesc{WF}\il{West Flemish}\\
...
An implied \textsc{agent} can be modified by an adjunct: in \REF{ex:key:04.19},
\emph{per ongeluk} (‘unintentionally’) or \emph{espres} (‘intentionally’)
modify the understood \textsc{agent}.\newpage

\ea%19
    \label{ex:key:04.19}\glsdesc{WF}\il{West Flemish}\\
...
\subsection{Event passive}\label{sec:key:04.2.4} %2.4. /
...
\textsc{goal} \isi{passive} \REF{ex:key:04.20}.

\ea%20
    \label{ex:key:04.20}\glsdesc{WF}\il{West Flemish}\\
...
\section{Conclusion}\label{sec:key:04.4} %3. /
