...
\title{Re-thinking re-categorisation: Is \enquote*{that} really a complementizer?}
...
\ea\label{ex:key:36.1}
	\textbf{That} guy over there gives me a headache (demonstrative)
\z

\ea\label{ex:key:36.2}
	Do you believe \textbf{that}? (anaphoric)
\z

\ea\label{ex:key:36.3}
	I believe [\textbf{that}\dots{}]  (complementizer)
\z
...
\ea\label{ex:key:36.4}
	I say \textbf{that}: [ main clause ] $\rightarrow$ I say [ \textbf{that}  embedded clause ]
\z
...
While Manzini and Savoia remain a bit vague about its actual status – besides
...
\textcite{BrandnerBucheli2018}, also \textcite{Axel2009,Axel2017}.}

The scenario in~\eqref{ex:key:36.4} would then look like the one
in~(4$'$).

\begin{exe}
    \exi{(4$'$)} I say (\textbf{that/it}) [ \textbf{that} \dots{}embedded clause (= relative clause)]
\end{exe}
...
\citep{Chomsky1981}, giving thus merely the impression of actual \isi{movement}.
...
\ea\label{ex:key:36.9} \ili{Irish}
    \ea
...
\ea\label{ex:key:36.10} \ili{Irish}\\
...
\ea\label{ex:key:36.11} \ili{Welsh}\\
...
the one in (\ref{ex:key:36.5}d). The following pattern is from \ili{Norwegian}
...
Alemannic \glspl{RC} (in this case specifically on Zuerich \ili{German}), that in case of
...
here only with a dative-argument and a subject-relativization:\is{dative case}

\ea\label{ex:key:36.13}Zuerich \ili{German}
    \ea
...
that they have both the same underlying syntax.\footnote{Recall that I assume
    ...
    acceptance of resumptives\is{resumptive pronouns} are given in table \eqref{tab:key:36.1}:
...
We varied the \isi{complementizers} and resumptives\is{resumptive pronouns} in the following way:
...
rather on the choice of the complementizer\is{complementizers}. Note that in table \eqref{ex:key:36.2}
...
\ea\label{ex:key:36.17} \ili{German}
...
\ea\label{ex:key:36.19} \ili{German}\\
